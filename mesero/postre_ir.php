...
<div class="wrapper"> 
 ...
 <form name="primerformulario" id="ff" method="post" action="envio_postre.php">
<fieldset>
<?php 
...
$result = mysqli_query($conexion,"SELECT  id_postre,nombre,precio FROM postre WHERE id_postre = $p") or die('Consulta fallida: ' . mysqli_error());

while ($row = mysqli_fetch_row($result)){   
  
  echo"<input type='radio' required checked name='group3' value=".$row[0].">".$row[0]."";
  echo "     <input type='text' size ='40' readonly value='".$row[1]."'></input>"; 
 echo "     Q. <input type='text' size='5' readonly value ='".$row[2]."'></input>"; 
   echo "<br>";
} 
echo "<center>
cantidad: <select name = 'cantidad_postres'>
<option>1</option>
<option>2</option>
<option>3</option>
<option>4</option>
